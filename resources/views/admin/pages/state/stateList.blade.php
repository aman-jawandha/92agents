        ...
        <div class="col-xs-12">
          ...
          <div class="box">
			...
			@if((isset(session('user_access_data')->statechange) && session('user_access_data')->statechange == 1) OR session("userid") == 1)
				<h3 class="box-title"><a class="btn btn-success" href="{{route('admin.state')}}">	
					<i class="fa fa-plus fa-xs"></i> Add State</a>			
				</h3>
			@endif
            </div>
            <!-- /.box-header -->
...
<script type="text/javascript">
  ...
  var table1 = $('#DataList').DataTable({ 	
		...
		"ajax": {	
			"url": "{{ url('/agentadmin/getStateList') }}",
		},
	...
	function confirm_function(id,messages)
	{
	    ...
	    BootstrapDialog.show({
			...
			buttons: [ {
						...
						dialogItself.close();
							$.ajax({
								url: "{{ url('/agentadmin/deleteState') }}",
								type: 'post',
