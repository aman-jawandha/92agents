...
@section('content')
    ...
    <section class="content-header">
      <h1>
        Certifications
        <small>Certifications list</small>
      </h1>
      <ol class="breadcrumb">
	  	<li><a href="{{route('admin.dashboard')}}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a href="{{route('admin.certifications')}}">Certifications</a></li>
        <li class="active">Certifications list</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
			...
			@if((isset(session('user_access_data')->certificationchange) && session('user_access_data')->certificationchange == 1) OR session("userid") == 1)
				<h3 class="box-title"><a class="btn btn-success" href="{{route('admin.certificationsaddedit')}}">	
					<i class="fa fa-plus fa-xs"></i> Add Certifications</a>			
				</h3>
			@endif
            </div>
            <!-- /.box-header -->
...
<script type="text/javascript">
  ...
  var table1 = $('#AreaList').DataTable({ 	
		...
		"ajax": {	
			"url": "{{ url('/agentadmin/getcertificationslist') }}",
			"headers": {
	...
	function confirm_function(id,messages)
	{
	    ...
	    BootstrapDialog.show({
			...
			buttons: [ {
						...
						dialogItself.close();
							$.ajax({
								url: "{{ url('/agentadmin/deletecertifications') }}", 
								type: 'post',
