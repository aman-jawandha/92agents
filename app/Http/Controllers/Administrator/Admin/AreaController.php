<?php

namespace App\Http\Controllers\Administrator\Admin;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\Redirect;
use App\Models\State;

class AreaController extends Controller
{

    /* For areas view in admin */
    public function areas()
    {
        $user = auth()->guard('admin')->user();
        return view('admin.pages.area.areaList');
    }

    /* Get area info */
    public function area($id = null)
    {
        $user = auth()->guard('admin')->user();
        $result = array();
        $state = new State;
        if (!empty($id)) :
            $result = $state->getAreaByAny(array('area_id' => $id), '1');
        endif;

        $view = array(
            'result' => $result
        );
        return view('admin.pages.area.area', $view);
    }

    /* For area save */
    public function save(Request $request)
    {

        $area_id = $request->input('area_id') ? $request->input('area_id') : '';
        $rules = array(
            'area'  => 'required|string|unique:agents_area,area_name',
        );

        if ($area_id !== "") {
            $rules = array(
                'area'  => "required|string|unique:agents_area,area_name,$area_id,area_id",
            );
        }
        $input_arr = array(
            'area' => $request->input('area'),
        );
        $validator = Validator::make($input_arr, $rules);
        if ($validator->fails()) :
            return Redirect::back()->withErrors($validator)->withInput();
        else :



            $data_arr = array(
                'area_name' => $request->input('area'),
                'is_deleted' => '0',
                'updated_at' => date('Y-m-d H:i:s')
            );
            if (!empty($area_id)) :
                DB::table('agents_area')->where(array('area_id' => $area_id))->update($data_arr);
                // return redirect('agentadmin/areas')->with('success','Area has been Inserted successfully.');
                return Redirect::back()->with('success', 'Area has been updated successfully.');
            else :
                $data_arr['created_at'] = date('Y-m-d H:i:s');
                DB::table('agents_area')->insertGetId($data_arr);
                // return redirect('agentadmin/areas')->with('success','Area has been Inserted successfully.');
                return Redirect::back()->with('success', 'Area has been created successfully.');
            endif;

            return Redirect::back()->with('dbError', 'Oops Something went wrong !!');
        endif;
    }

    /* For date and time */
    public function mmddyyy($date = Null)
    {
        $formate = "";
        if (!empty($date) && $date != "0000-00-00 00:00:00") :
            $formate = date('M d Y', strtotime($date));
        endif;
        return $formate;
    }

    /* get area list info */
    public function getAreaList()
    {

        $state = new State;
        $list = $state->getAreaList($_REQUEST, $_REQUEST['length'], $_REQUEST['start']);
        $data = array();
        $no = $_REQUEST['start'];
        foreach ($list['result'] as $result) {
            $no++;
            $row = array();
            $row[] = $no;
            $row[] = isset($result->area_name) ? ucwords(strtolower($result->area_name)) : '';
            $row[] = isset($result->created_at) ? $this->mmddyyy($result->created_at) : '';

            if ((isset(session('user_access_data')->areachange) && session('user_access_data')->areachange == 1) or session("userid") == 1) {
                $row[] =  $result->status == 1 ? '<button class="btn btn-success" onClick ="status_change_function(\'' . $result->area_id . '\',0,\'Are you sure want to deactive this record ? \');">Active</button>' :
                    '<button class="btn btn-danger" onClick ="status_change_function(\'' . $result->area_id . '\',1,\'Are you sure want to active this record ? \');">Deactive</button>';
            } else {
                $row[] =  $result->status == 1 ? '<button class="btn btn-success">Active</button>' : '<button class="btn btn-danger">Deactive</button>';
            }

            if ((isset(session('user_access_data')->areachange) && session('user_access_data')->areachange == 1) or session("userid") == 1) {
                $row[] =  '<a class="btn btn-success" href="area/' . $result->area_id . '">
						<i class="fa fa-pencil fa-xs"></i></a>
						<button class="btn btn-danger" onClick ="confirm_function(\'' . $result->area_id . '\',\'Are you sure, you want to delete this record? \');">
						<i class="fa fa-trash-o fa-xs"></i></button>';
            } else {
                $row[] = 'No access';
            }
            $data[] = $row;
        }

        $output = array(
            "draw" => isset($_REQUEST['draw']) ? intval($_REQUEST['draw']) : '',
            "recordsTotal" => intval($list['num']),
            "recordsFiltered" => intval($list['num']),
            "data" => $data,
        );
        echo json_encode($output);
    }

    /* For delete areas */
    public function deleteArea(Request $request)
    {

        $id = $request->input('id');
        $tag = $request->input('tag');
        if (!empty($id)) {
            if ($tag == 'Delete') :
                DB::table('agents_area')->where(array('area_id' => $id))->update(array('is_deleted' => '1'));
            elseif ($tag == 'status') :
                $value = $request->input('value');
                DB::table('agents_area')->where(array('area_id' => $id))->update(array('status' => $value));
            endif;
        }
    }
}
